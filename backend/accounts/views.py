...
class CreateAccountView(APIView):
    def post(self, request):
        with tracer.start_as_current_span("create_account"):
            span = trace.get_current_span()
            span.set_attribute("http.method", request.method)
            span.set_attribute("http.route", "/accounts")
...
class UserAccountsView(APIView):
    def get(self, request, userId):
        with tracer.start_as_current_span("get_user_accounts"):
            span = trace.get_current_span()
            span.set_attribute("http.method", request.method)
            span.set_attribute("http.route", "/accounts/user/<userId>")
...
class AccountDetailView(APIView):
    def get(self, request, accountId):
        with tracer.start_as_current_span("get_account_detail"):
            span = trace.get_current_span()
            span.set_attribute("http.method", request.method)
            span.set_attribute("http.route", "/accounts/<accountId>")
    ...
    def put(self, request, accountId):
        with tracer.start_as_current_span("update_account_detail"):
            span = trace.get_current_span()
            span.set_attribute("http.method", request.method)
            span.set_attribute("http.route", "/accounts/<accountId>")
            account = get_object_or_404(Account, account_id=accountId)
    ...
    def delete(self, request, accountId):
        with tracer.start_as_current_span("delete_account"):
            span = trace.get_current_span()
            span.set_attribute("http.method", request.method)
            span.set_attribute("http.route", "/accounts/<accountId>")
            account = get_object_or_404(Account, account_id=accountId)
